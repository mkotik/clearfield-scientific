...
// Top hero panel with a scroll-scrubbed 3D video background: the section is taller
// than the viewport; while it's pinned, page-scroll drives video.currentTime, so
// scrolling "moves the camera" through the 3D scene behind the hero copy. The video
// is re-encoded all-keyframes for exact, cheap seeks; no React state changes on
// scroll (avoids re-render lag) and the layout read + seek are batched into one rAF.
export default function Showcase3D() {
  ...
  useEffect(() => {
    ...
    if (reduce) return // leave the poster / first frame; no scroll-driven motion
  ...
  return (
    <section id="showcase" ref={sectionRef} style={{ position: 'relative', height: '300vh', background: '#0B2545' }}>
      <div style={{ position: 'sticky', top: 0, height: '100vh', overflow: 'hidden' }}>
        <video
          ref={videoRef}
          src="/clearpoint-3d.mp4"
          poster="/clearpoint-3d-poster.jpg"
          muted
          playsInline
          preload="auto"
          tabIndex={-1}
        ...
        <Crosshair top={88} right={26} size={18} color="rgba(255,255,255,0.6)" />
        <Crosshair bottom={24} left={26} size={18} color="rgba(255,255,255,0.6)" />
        <Crosshair bottom={24} right={26} size={18} color="rgba(255,255,255,0.6)" />

        {/* hero copy + CTAs */}
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center' }}>
          <div style={{ width: '100%', maxWidth: 'var(--maxw)', margin: '0 auto', padding: '0 var(--pad-x)' }}>
            <div className="cp-fade-up" style={{ maxWidth: 640, animation: 'cpFadeUp 0.7s ease both' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                <LocatingDot pulse ringColor="#9CC2F2" dotColor="#9CC2F2" ringOpacity={0.7} />
                <span
                  style={{
              ...
              <h1
                style={{
                  ...
                  color: '#fff',
                  margin: '0 0 26px',
                  textShadow: '0 2px 34px rgba(7,18,33,0.7), 0 1px 4px rgba(7,18,33,0.6)',
                }}
              >
                Scientific Supplies.
                <br />
